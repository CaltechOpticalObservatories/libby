...
concatenate them by hand. `group`/`scope` are case-insensitive
(`HSFEI.ADC` and `hsfei.adc` reach the same peer); the keyword name isn't.
Cross-peer fanout is not supported. `req` and `sub` are kept for raw RPC /
topic debugging.
